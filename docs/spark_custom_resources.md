...
* While the `Workload` waits for quota, the resource stays in its initializing state (`Submitted`,
  ...
  `kueue.x-k8s.io/queue-name` label changes while waiting, the `Workload` is moved to the new queue
  in place, since it holds no quota yet. If the label is removed while waiting, the `Workload`
  is deleted, so that Kueue does not admit it later into quota which nothing uses, and the
  resource starts without Kueue.
