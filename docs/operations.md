...
| nameOverride                                                     | Override the chart name used in the names and labels of the created resources.                                                                                                 |                                                                                                         |
...
| operatorDeployment.networkPolicy.metricsIngress                  | List of NetworkPolicyPeer(s) allowed to reach the metrics port, e.g. the Prometheus scraper. When empty, ingress to the metrics port is denied.                                |                                                                                                         |
...
| operatorDeployment.operatorPod.affinity                          | Custom affinity to be added to the operator pod.                                                                                                                               |                                                                                                         |
| operatorDeployment.operatorPod.nodeSelector                      | Custom nodeSelector to be added to the operator pod.                                                                                                                           |                                                                                                         |
| operatorDeployment.operatorPod.tolerations                       | Custom tolerations to be added to the operator pod.                                                                                                                            |                                                                                                         |
...
| operatorDeployment.operatorPod.dnsPolicy                         | DNS policy to be used by the operator pod.                                                                                                                                     |                                                                                                         |
...
| operatorDeployment.operatorPod.operatorContainer.jvmArgs         | JVM arg override for the operator container.                                                                                                                                   | `"-Dfile.encoding=UTF8 -XX:+CrashOnOutOfMemoryError -XX:ErrorFile=/dev/stderr -XX:+UseParallelGC -XX:InitialRAMPercentage=80 -XX:MaxRAMPercentage=80 -XX:+AlwaysPreTouch -XX:+UseCompactObjectHeaders"` |
...
| operatorDeployment.operatorPod.operatorContainer.volumeMounts    | Additional volume mounts to be added to the operator container.                                                                                                                |                                                                                                         |
| operatorDeployment.operatorPod.operatorContainer.probes          | Probe config for the operator container.                                                                                                                                       |                                                                                                         |
| operatorDeployment.operatorPod.operatorContainer.metrics.port    | Metrics port of the operator container.                                                                                                                                        | 19090                                                                                                   |
...
| operatorDeployment.operatorPod.additionalContainers              | Additional containers to be added to the operator pod, e.g. sidecar.                                                                                                           |                                                                                                         |
| operatorRbac.serviceAccount.create                               | Whether to create service account for operator to use.                                                                                                                         | true                                                                                                    |
| operatorRbac.serviceAccount.name                                 | Name of the operator ServiceAccount.                                                                                                                                           | `"spark-operator"`                                                                                      |
...
| workloadResources.serviceAccount.create                          | Whether to create a service account for Spark workload.                                                                                                                        | true                                                                                                    |
| workloadResources.serviceAccount.name                            | The name of Spark workload service account.                                                                                                                                    | `spark`                                                                                                 |
...
| workloadResources.sparkApplicationSentinel.sentinelNamespaces.data | A list of namespaces where sentinel resources will be created in. Note that these namespaces have to be a subset of `workloadResources.namespaces.data`.                       |                                                                                                         |
...
| operatorConfiguration.dynamicConfig.source                       | Source of the dynamic config: `configMap` (watched via informer, requires RBAC to read config maps) or `file` (periodically reloaded from the mounted config map).             | `configMap`                                                                                             |
...
4. Set `workerSpec.metricsPort` to the same port, so the operator's generated `NetworkPolicy`
   admits ingress on it from any source, in addition to the existing cluster/driver label
   allow-list:
...
expose master metrics, with no equivalent of `metricsPort` needed.
