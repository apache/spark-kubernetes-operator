...
class SparkAppReconcilerTest {
  ...
  void testCleanupRunningApp() {
    try (MockedConstruction<SparkAppContext> mockAppContext =
        ...
        MockedStatic<SparkReconcilerUtils> utils = Mockito.mockStatic(SparkReconcilerUtils.class)) {
      // delete running app
      app.setStatus(
          app.getStatus()
      ...
      utils.verify(
          () -> SparkReconcilerUtils.deleteResourceIfExists(mockClient, mockDriver, false));
