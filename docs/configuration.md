...
| `KueueEvictionIgnored` | Kueue evicted the `Workload` of a running `SparkCluster` by a `PodsReadyTimeout`, which the operator does not act on, so the master and workers keep running and holding its quota. It is republished on every reconciliation while the eviction lasts, subject to [`minIntervalSeconds`](#event-frequency). See [Kueue](spark_custom_resources.md#kueue). |
