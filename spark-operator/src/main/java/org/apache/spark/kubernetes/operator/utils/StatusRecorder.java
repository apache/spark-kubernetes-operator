...
import org.apache.spark.kubernetes.operator.controller.BaseContext;
import org.apache.spark.kubernetes.operator.listeners.BaseStatusListener;
 ...
 */
@Slf4j
public class StatusRecorder<STATUS extends BaseStatus<?, ?, ?>,
  ...
  protected StatusRecorder(List<LISTENER> statusListeners,
  ...
  private void patchAndStatusWithVersionLocked(CR resource, KubernetesClient client) {
    ...
    Exception err = null;
    long maxRetry = StatusPatchMaxRetry.getValue();
  ...
  public void persistStatus(BaseContext<CR> context,
                            STATUS newStatus) {
  ...
  public void updateStatusFromCache(CR resource) {
    ...
    if (cachedStatus != null) {
      resource.setStatus(
          objectMapper.convertValue(cachedStatus, statusClass));
    } else {
