...
package org.apache.spark.k8s.operator.health;
...
class SentinelManagerTest {
  ...
  void testIsSentinelResource() {
    ...
    try (MockedStatic<SparkReconcilerUtils> mockUtils = mockStatic(SparkReconcilerUtils.class)) {
  ...
  void testHandleSentinelResourceReconciliation() throws InterruptedException {
    ...
    kubernetesClient.resource(SparkReconcilerUtils.clone(mockApp)).create();
