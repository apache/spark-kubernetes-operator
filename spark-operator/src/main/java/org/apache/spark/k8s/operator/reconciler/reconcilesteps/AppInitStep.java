...
public class AppInitStep extends AppReconcileStep {
    ...
    try {
      ...
      for (HasMetadata resource : context.getDriverPreResourcesSpec()) {
        Optional<HasMetadata> createdResource =
            SparkReconcilerUtils.getOrCreateSecondaryResource(context.getClient(), resource);
      ...
      Optional<Pod> driverPod =
          SparkReconcilerUtils.getOrCreateSecondaryResource(
  ...
  private void updateStatusForCreationFailure(
      SparkAppContext context, HasMetadata resourceSpec, SparkAppStatusRecorder statusRecorder) {
    if (log.isErrorEnabled()) {
      log.error("Failed all attempts to request driver resource {}.", resourceSpec.getMetadata());
    }
    statusRecorder.persistStatus(
        context,
        context
