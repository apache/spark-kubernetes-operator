    ...
    if (StringUtils.isNotBlank(listenerNamesStr)) {
      try {
        List<String> listenerNames =
            Arrays.stream(listenerNamesStr.split(","))
                .map(String::trim)
                .collect(Collectors.toList());
        for (String name : listenerNames) {
          Class listenerClass = Class.forName(name);
          if (SparkAppStatusListener.class.isAssignableFrom(listenerClass)) {
            listeners.add((SparkAppStatusListener) listenerClass.getConstructor().newInstance());
          }
        }
      } catch (Exception e) {
        if (log.isErrorEnabled()) {
          log.error("Failed to initialize listeners for operator with {}", listenerNamesStr, e);
        }
      }
    }
    return listeners;
